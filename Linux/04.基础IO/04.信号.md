...
例如，当一个变量可能被多个线程或中断处理程序修改时，使用 `volatile` 可以确保每次访问都会得到最新的值，而不会使用缓存中的旧值。

### 可重入函数

当函数执行时，如果因为发生信号，产生了一个新执行流。这时候进程将会去执行处理信号的方法，处理结束后，将会返回位置。

此时称作函数被重入。

如果函数因为被重入，导致出现了报错或者隐含的问题（如内存泄漏），这个函数称作博客插入函数。

如果函数不会因为重入出现问题，则称为可重入函数。

大部分函数都是不可重入函数。

如果函数不涉及全局数据，且相对独立，则大概率为可重入函数。
### 核心转储
